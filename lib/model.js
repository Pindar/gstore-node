...
class Model extends Entity {
    static compile(kind, schema, gstore) {
        ...
        function metaData() {
            ...
            Object.keys(schema.paths).forEach((k) => {
                ...
                if (schema.paths[k].type === 'key') {
                    meta.keyProps = meta.keyProps || [];
                    meta.keyProps.push(k);
                }
            });
    ...
    static deleteAll(ancestors, namespace, cb) {
        ...
        return query.run().then(onEntities, onError);
    ...
    save(transaction, options, cb) {
         ...
         * - convert array with type key to Datastore Key Object
         */
        function prepareData() {
            updateModifiedOn.call(this);
            convertGeoPoints.call(this);
            convertKeys.call(this);
            ...
            */
            ...
            /**
             * If the entityData has some property of type 'key'
             * and its value is an js array with minimum two entries
             * we convert it to a datastore Key.
            */
            function convertKeys() {
                if (!{}.hasOwnProperty.call(this.schema.__meta, 'keyProps')) {
                    return;
                }
                this.schema.__meta.keyProps.forEach((property) => {
                    if ({}.hasOwnProperty.call(_this.entityData, property) &&
                        _this.entityData[property] !== null &&
                        _this.entityData[property].constructor.name !== 'Key') {
                        _this.entityData[property] = _this.gstore.ds.key(_this.entityData[property]);
                    }
                });
            }
        }
        ...
        Object.keys(this.entityData).forEach((k) => {
            ...
            if (!skip && schemaHasProperty && !isValueEmpty && {}.hasOwnProperty.call(schema.paths[k], 'type')) {
                ...
                if (propertyType === 'datetime') {
                    ...
                    } else if (propertyType === 'key') {
                        // Key
                        typeValid = is.array(propertyValue) && propertyValue.length > 1;
                    } else {
                        // Other
